...
def kp_detection(
    ...
    for ind in tqdm(range(0, num_images), ncols=67, desc="locating kps"):
        ...
        for scale in multi_scales:
            ...
            orig_target_sizes = torch.tensor(input_size).unsqueeze(0)
            ...
            if isDecAttn or isEncAttn:
                hooks = [
                    ...
                    nnet.model.module.transformer.encoder.layers[
                        -1
                    ].self_attn.register_forward_hook(
                        lambda self, input, output: enc_attn_weights.append(output[1])
                    ),
                    nnet.model.module.transformer.decoder.layers[
                        -1
        ...
        if debug:
            ...
            if isDecAttn:
                ...
                keep = results[0, :, 0].cpu() == 1.0
                fig, axs = plt.subplots(
                    ncols=keep.nonzero().shape[0] + 1, nrows=2, figsize=(44, 14)
                )
                ...
                for idx, ax_i in zip(keep.nonzero(), axs.T):
                    ...
                    ax = ax_i[1]
                    preds = db.draw_annotation(
                        ind,
                        pred=results[0][idx].cpu().numpy(),
                        cls_pred=None,
                        img=raw_img,
                    )
                    ax.imshow(preds)
            ...
            if isEncAttn:
                ...
                for i, lane in enumerate(label):
                    ...
                    for idx_o, idx, num in zip(idxs, attn_idxs, range(xs.shape[0])):
                        ...
                        ax.add_patch(
                            plt.Circle(
                                (int(idx_o[1]), int(idx_o[0])), color="r", radius=16
                            )
                        )
                        ax.axis("off")
                        fig.tight_layout()
            ...
            if not isEncAttn and not isDecAttn:
                preds = db.draw_annotation(
                    ind, pred=results[0].cpu().numpy(), cls_pred=None, img=image
                )
                cv2.imwrite(
                    os.path.join(
                        lane_debug_dir,
