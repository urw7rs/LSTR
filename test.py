...
def parse_args():
    ...
    parser.add_argument("--modality", dest="modality", default=None, type=str)
...
def test(
    ...
    if modality == "eval":
    ...
    elif modality == "opencv":
        print("testing plain opencv lane detection...")
        test_file = "test.opencv"
        testing = importlib.import_module(test_file).testing
        testing(
            db,
            nnet=None,
        ...
        raise ValueError(
            "--modality must be one of eval/images, but now: {}".format(modality)
        )
...
if __name__ == "__main__":
    ...
    if args.suffix is None:
        ...
        cfg_file = os.path.join(
            system_configs.config_dir, args.cfg_file + "-{}.json".format(args.suffix)
        )
